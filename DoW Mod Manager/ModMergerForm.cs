...
namespace DoW_Mod_Manager
{
    public partial class ModMergerForm : Form
    {
        ...
        private void HideUnavailableMods()
        {
            ...
            foreach (Mod mod in modlist)
            {
                if (AvailableModsList.Items.Contains(mod.Name))
                    AvailableModsList.Items.Remove(mod.Name);
            }
        }
        ...
        private void SortInactiveModsToBottom()
        {
            ...
            for (int i = 0; i < modlist.Count; i++)
            {
                if (modlist[i].State.Equals(MOD_INACTIVE))
                {
                    ...
                    modlist.RemoveAt(i);
                    i--;                                    // Go one step Back to stay in place for next entry
                }
            }
